...
def main(argv=None):
    ...
    parser.add_option(
        ...
        "--track",
        dest="track",
        help="Restrict analysis to one of the input samples vs." "all the others",
        default=None,
    )
    ...
    running_totals = {
        sf: [collections.defaultdict(int) for x in range(len(args) - 1)] for sf in args
    }
    running_hits = {
        sf: [collections.defaultdict(int) for x in range(len(args) - 1)] for sf in args
    }
    ...
    if options.track:
        use_index, use_names = zip(
            *[(i, fn) for i, fn in enumerate(args) if fn == options.track]
        )
    else:
    ...
    E.debug(
        "Reporting on input(s) %s: %s"
        % (",".join(map(str, use_index)), ",".join(use_names))
    )
    ...
    for ref, length in contigs:
        ...
        for sf in range(len(samfiles)):
            ...
            pos_depth, neg_depth, counter = count.countChr(
                samfiles[sf].fetch(ref), length, options.dtype
            )
        ...
        for sf in use_index:
            try:
                E.debug(
                    "Max depth for %s is %i" % (args[sf], int(depths.iloc[:, sf].max()))
                )
            except ValueError:
            ...
            for n in range(n_max):
                E.debug(
                    "Calculating %i level reproducibility for file %s" % (n, args[sf])
                )
                ...
                replicating_sites = depths.ix[sites, np.arange(len(samfiles)) != sf] > 0
                n_replicating_samples = replicating_sites.sum(axis=1)

                for i in range(len(args) - 1):
                    running_hits[args[sf]][i][n] += (n_replicating_samples > i).sum()
                del sites
